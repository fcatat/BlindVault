...
collect_config() {
  ...
  echo ""
  info "LLM settings ${DIM}(optional — can be configured later in the web UI)${NC}"
  echo ""
  echo -ne "  ${CYAN}?${NC}  Configure LLM now? ${DIM}(y/N)${NC}: "
  ...
  if [[ "${configure_llm,,}" == "y" || "${configure_llm,,}" == "yes" ]]; then
    ask "LLM provider"  "openai"  LLM_PROVIDER
    ask "Model name"    "gpt-4o"  LLM_MODEL
    ask "API Base URL"  ""        LLM_BASE_URL
    ask_secret "API Key (hidden input)" LLM_API_KEY
    echo ""
    success "LLM configured: ${LLM_PROVIDER} / ${LLM_MODEL}"
  else
    echo ""
    info "Skipped — you can set it up at ${BOLD}http://localhost:${FRONTEND_PORT}${NC} → Agent Config"
  fi
}
...
# LLM (can be changed later in web UI → Agent Config)
...
print_success() {
  ...
  if [[ "$LLM_PROVIDER" == "mock" ]]; then
    echo -e "${GREEN}║${NC}  ${DIM}💡 LLM not configured yet. Set it up at:${NC}   ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}  ${DIM}   Web UI → Agent Config${NC}                    ${GREEN}║${NC}"
    echo -e "${GREEN}║${NC}                                              ${GREEN}║${NC}"
  fi
