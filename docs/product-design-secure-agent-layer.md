...
> - **两个拦截点正好是内置 middleware**：`HumanInTheLoopMiddleware`（可恢复审批，需 checkpointer）+ 自定义 `AgentMiddleware`（单层可逆脱敏，回写金库。未来不可逆 PII 阻断兜底由 LiteLLM 接管）；durable runtime 的暂停-持久化-恢复是业界最成熟；
...
**② 兜底层 — PII 兜底移至 LiteLLM 网关层（路线图）**
LiteLLM 内置 Presidio PII masking guardrail，支持自定义正则识别器（ad-hoc recognizer JSON）、`MASK`/`BLOCK` 两种动作、置信度阈值。未来不可逆兜底将完全由独立网关接管，应用内不再保留 Presidio 或正则兜底逻辑。
- 作用：凡是主层漏掉、但又像密码/密钥/连接串的，**直接 BLOCK 整个请求**（最安全）或 MASK 成 `<SECRET>`。
- 这是**不依赖应用正确性的失效保险**：哪怕你的应用层主层脱敏有 bug，独立网关兜底层仍拦住泄露。
- 它不回写你的金库，所以它**只做兜底，不做主力** —— 角色定位要清晰。
...
| Presidio 容器 ×2 | 不可逆兜底识别（路线图：移至 LiteLLM 网关侧） | 引入开源 Presidio |
...
presidio-analyzer     # 兜底识别 (路线图)
presidio-anonymizer   # 兜底脱敏 (路线图)
redis                 # 金库热存储
...
5. **脱敏漏报 = 泄露**（老问题，未消失）：主层单层防护下，识别率是命门。「未知格式的密钥」永远是残余风险。未来在 LiteLLM 网关侧引入 PII 兜底（路线图）后，建议默认对兜底层启用 **BLOCK** 而非 MASK，宁可拦错不可放过。
...
**V1（再 3–4 周）**
- 在 LiteLLM 网关层引入 Presidio PII 兜底（BLOCK 模式）+ 自定义凭证识别器（路线图）。
