...
10. 多步骤计划 (Task Planning)：当用户给出的运维请求包含多个步骤，或者属于一个复合的、需要执行多条 Shell 命令的流程时，你必须调用 `generate_task_plan` 工具来先生成一份执行计划，将步骤拆解规划出来。不要尝试在一大段 secure_shell 中跑完所有不相关的复杂任务。
11. 定时计划任务 (Scheduled Tasks)：当用户要求在未来延迟执行，或者周期性定时执行任务时，你必须调用 `create_scheduled_task` 工具在后台调度该任务，向用户反馈创建成功与下次运行时间。

可用工具：
- secure_shell: 通用安全命令执行器
  - command: Shell 命令，用 $SECRET 代替密码
  - secret_ref: 密码引用 (sec_live_xxx)
- generate_task_plan: 运维步骤拆解规划器
  - steps: 包含多步操作的列表，每个步骤包含 title, command, 以及可选的 secret_ref。
...
- 计划任务部署: generate_task_plan(steps=[{"title": "拉取Nginx镜像", "command": "docker pull nginx"}, {"title": "启动容器", "command": "docker run -d -p 80:80 nginx"}])
"""
...
def _mock_chatbot(state: AgentState) -> dict:
    ...
    # 多步计划关键词匹配
    is_plan = bool(re.search(r"plan|步骤|部署|复合", content, re.IGNORECASE))
    if is_plan:
        return {
            "messages": [
                AIMessage(
                    ...
                    tool_calls=[
                        {
                            ...
                            "args": {
                                "steps": [
                                    {"title": "第1步: 检查运行状态", "command": "docker ps"},
                                    {"title": "第2步: 查看系统负载", "command": "uptime"}
                                ]
                            }
                        }
                    ]
...
def _create_openai_chatbot():
    ...
    from backend.tools.task_plans import GENERATE_TASK_PLAN_SCHEMA, CREATE_SCHEDULED_TASK_SCHEMA
    ...
    tools_schema = [
        ...
        },
        {
            ...
            "function": {
                "name": "generate_task_plan",
                "description": "用于多步骤、复合的复杂运维任务步骤拆解规划。AI 将先输出执行步骤，等待用户核对或在前端逐步执行。",
        ...
        if isinstance(msg, AIMessage):
            ...
            if msg.tool_calls:
                for tc in msg.tool_calls:
                    ...
                    if tc["name"] == "generate_task_plan":
                        steps = tc["args"].get("steps", [])
                        plan_steps = []
                        for i, st in enumerate(steps):
                            plan_steps.append({
                                "index": i + 1,
                                "title": st.get("title", f"步骤 {i+1}"),
                                "command": st.get("command", ""),
                                "secret_ref": st.get("secret_ref"),
                                "status": "pending",
                            })
                        plan_data = {
