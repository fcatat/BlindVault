...
- **两个拦截点**（整个产品的核心）：
  - **拦截点 A — 出站脱敏**：一切流向模型的内容（输入 + 文件 + 命令回显）必须无密码。落点：自定义 `AgentMiddleware.before_model`（可逆，回写金库）+ 内置 `PIIMiddleware`（不可逆兜底）。
  - **拦截点 B — 执行注入 + 审批**：执行瞬间 resolve 占位符为真密码；高危命令暂停等人工确认。落点：`secure_shell` 工具内 `resolve_secret` + 内置 `HumanInTheLoopMiddleware`（需 checkpointer）。
...
2. **可逆脱敏只走自有金库**：内置 `PIIMiddleware` 不可逆，**只作 block 兜底**，绝不当主层。
